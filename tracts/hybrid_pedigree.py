...
def prob_of_pop_setting(ms, all_possible_migrations_list, migrations_at_T_list, f_migrations, m_migrations, parent_sex, number_ind, number_anc, Number_pops, pedigree):
    ...
    mig_setting = np.concatenate([sex_and_gen[~np.isnan(migrant_individuals),:], migrant_individuals[~np.isnan(migrant_individuals),np.newaxis], np.zeros([np.sum(~np.isnan(migrant_individuals)), 1])], axis = 1)
...
def density_hybrid_pedigree(which_migration, migration_list, T_PED, which_pop, D_model, which_L, bins, mmat_f, mmat_m, rrr_f, rrr_m, is_X_chr, is_X_chr_male):
    ...
    if np.all(ancestral_setting == which_pop + 1):
        ...
        PhT_ped = PhT_Dioecious(migration_matrix_f = mmat_f, migration_matrix_m = mmat_m, rho_f = rrr_f, rho_m = rrr_m, 
        ...
        if np.all(PhT_ped.source_populations_f == which_pop):
            ...
            newbins, counts_f, ETL_f = PhaseTypeDistribution.tractlength_histogram_windowed(PhT_ped, which_pop, bins, L = which_L, density = True, return_only = 1, freq = False)
        ...
        if np.all(PhT_ped.source_populations_m == which_pop):
            ...
            if not is_X_chr_male:
                newbins, counts_m, ETL_m = PhaseTypeDistribution.tractlength_histogram_windowed(PhT_ped, which_pop, bins, L = which_L, density = True, return_only = 0, freq = False)
...
def Ped_DF_density(mig_matrix_f, mig_matrix_m, TP, L, bingrid, whichpop, Dioecious_model = 'DF', all_possible_migrations_TP = None, rr_f = 1, rr_m = 1, X_chr = False, X_chr_male = False, N_cores = 1, freq = False):
    ...
    t0_proportions = np.sum((0.5*(mig_matrix_f + mig_matrix_m))*np.transpose(survival_factors)[:,np.newaxis], axis = 0)    
    ...
    prob_of_pop_setting_iteration_0 = partial(prob_of_pop_setting, all_possible_migrations_list = all_possible_migrations_TP, 
                                                migrations_at_T_list = migrations_at_TP, f_migrations = mig_matrix_f, 
                                                m_migrations = mig_matrix_m, parent_sex = 0, number_ind = nind_TP, number_anc = nanc_TP, 
                                                Number_pops = Npops, pedigree = the_pedigree)
       
    prob_of_pop_setting_iteration_1 = partial(prob_of_pop_setting, all_possible_migrations_list = all_possible_migrations_TP, 
                                                migrations_at_T_list = migrations_at_TP, f_migrations = mig_matrix_f, 
                                                m_migrations = mig_matrix_m, parent_sex = 1, number_ind = nind_TP, number_anc = nanc_TP, 
                                                Number_pops = Npops, pedigree = the_pedigree)
    ...
    prob_list_m_complete = Parallel(n_jobs = N_cores, verbose = 10, prefer = 'processes')(delayed(prob_of_pop_setting_iteration_0)(i) for i in range(len(all_possible_migrations_TP)))        
    ...
    prob_list_f_complete = Parallel(n_jobs = N_cores, verbose = 10, prefer = 'processes')(delayed(prob_of_pop_setting_iteration_1)(i) for i in range(len(all_possible_migrations_TP)))        
    ...
    densities_list = Parallel(n_jobs = N_cores, verbose = 10, prefer = 'processes')(delayed(density_hybrid_iteration)(i) for i in range(len(migrations_at_TP)))        
    ...
    if X_chr and X_chr_male:
        ...
        final_density = np.sum(np.array([l[0]*prob_list_f[prob_list_f[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_f[:,0])]), axis = 0)
        Exp = np.sum(np.array([l[3]*prob_list_f[prob_list_f[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_f[:,0])]), axis = 0)
        scale = t0_proportions[whichpop]*L/Exp if freq else 1
        
    else:
        
        final_density = 0.5*(np.sum(np.array([l[0]*prob_list_f[prob_list_f[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_f[:,0])]), axis = 0) + np.sum(np.array([l[1]*prob_list_m[prob_list_m[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_m[:,0])]), axis = 0))    
        Exp = 0.5*(np.sum(np.array([l[3]*prob_list_f[prob_list_f[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_f[:,0])]), axis = 0) + np.sum(np.array([l[4]*prob_list_m[prob_list_m[:,0] == l[2], 1] for l in densities_list if np.isin(l[2], prob_list_m[:,0])]), axis = 0))    
